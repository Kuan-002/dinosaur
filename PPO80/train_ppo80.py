...
def replay_actions(
    ...
    for step in range(actions.size(1)):
        ...
        if model.cfg.decoupled_stop_policy:
            stop_logits = model.stop_head(h.detach()).squeeze(-1)
            stop_dist = Bernoulli(logits=stop_logits)
            valid_stop = active & can_stop & is_stop
            slot_logits = model.slot_policy_logits(h.detach(), slot_embeds.detach(), selected, step)
            slot_dist = Categorical(logits=slot_logits)
            slot_action = action.clamp(min=0, max=k - 1)
            step_log_prob = stop_dist.log_prob(valid_stop.to(dtype=stop_logits.dtype)) * active_f
            step_log_prob = step_log_prob + slot_dist.log_prob(slot_action) * (active & ~valid_stop).to(slots.dtype)
            step_entropy = (stop_dist.entropy() + slot_dist.entropy() * (~valid_stop).to(slots.dtype)) * active_f
            is_stop = valid_stop | (~active)
        else:
            logits = model.policy_logits(h.detach(), slot_embeds.detach(), selected, step)
            ...
            step_log_prob = dist.log_prob(action) * active_f
            step_entropy = dist.entropy() * active_f
            is_stop = (action == model.stop_idx) & active
...
def ppo_train_step_slothead_reward(
    model: SlotSelectorGRPO,
    ...
    with torch.no_grad():
        old_out = rollout_slothead_reward(model, slots, reward_features, args, sample=True)
        old_value = value_head(old_out["final_state"]).detach()
        old_log_probs = old_out["log_probs"].detach()
        old_mask = old_out["mask"].detach()
        returns = old_out["trajectory_reward"].detach()
        advantages = returns - old_value
        if bool(args.normalize_advantages) and advantages.numel() > 1:
            advantages = (advantages - advantages.mean()) / advantages.std(unbiased=False).clamp_min(float(args.ppo_adv_eps))

    active_steps = old_mask.sum().clamp_min(1.0)
    clip_eps = float(args.ppo_clip_eps)
    log_ratio_clip = float(args.ppo_log_ratio_clip)
    total_loss_value = 0.0
    out = old_out
    for _ in range(int(args.ppo_epochs)):
        replay = replay_actions(model, slots, old_out["actions"], args)
        values = value_head(replay["final_state"].detach())
        log_ratio = (replay["log_probs"] - old_log_probs).clamp(-log_ratio_clip, log_ratio_clip)
        ratio = torch.exp(log_ratio)
        adv_steps = advantages[:, None]
        unclipped = ratio * adv_steps
        clipped = ratio.clamp(1.0 - clip_eps, 1.0 + clip_eps) * adv_steps
        policy_loss = (-torch.minimum(unclipped, clipped) * old_mask).sum() / active_steps
        value_loss = F.mse_loss(values, returns)
        entropy_loss = -((replay["entropies"] * replay["mask"]).sum() / active_steps)
        loss = (
            float(args.policy_coef) * policy_loss
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(grpo.policy_parameters(model) + list(value_head.parameters()), args.grad_clip)
        policy_optimizer.step()
        value_optimizer.step()
        total_loss_value += float(loss.detach().cpu())
        out = {**old_out, **replay, "terminal_reward": returns, "trajectory_reward": returns}

    replay = replay_actions(model, slots, old_out["actions"], args)
    cls_loss = F.cross_entropy(replay["logits"], labels, label_smoothing=float(args.label_smoothing))
    cls_loss = float(args.classification_coef) * cls_loss
    ce_optimizer.zero_grad(set_to_none=True)
    cls_loss.backward()
    nn.utils.clip_grad_norm_(grpo.ce_parameters(model), args.grad_clip)
    ce_optimizer.step()
    total_loss_value += float(cls_loss.detach().cpu())
    out = {**old_out, **replay, "terminal_reward": returns, "trajectory_reward": returns}
    out["value"] = value_head(out["final_state"]).detach()
    out["advantage"] = advantages.detach()
    return out, slots.new_tensor(total_loss_value / (max(int(args.ppo_epochs), 1) + 1))
...
def run_epoch(
    ...
    for images, labels in loader:
        ...
        if train and args.warmup_epochs_remaining > 0:
            ...
            out, loss = ppo_train_step_slothead_reward(
                model,
                value_head,
                slots,
                reward_features,
        ...
        if train and not stepped:
            ...
            value_optimizer.zero_grad(set_to_none=True)
            loss.backward()
            nn.utils.clip_grad_norm_(list(model.parameters()) + list(value_head.parameters()), args.grad_clip)
            ce_optimizer.step()
            policy_optimizer.step()
            value_optimizer.step()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train RNN-PPO selector over 80-dim slothead features.")
    ...
    parser.add_argument("--output_dir", default="PPO80/checkpoints/ppo80")
    ...
    parser.add_argument("--ppo_log_ratio_clip", type=float, default=10.0)
    parser.add_argument("--ppo_adv_eps", type=float, default=1e-4)
    parser.add_argument("--value_coef", type=float, default=0.5)
    parser.add_argument("--normalize_advantages", action="store_true", default=True)
    parser.add_argument("--disable_normalize_advantages", dest="normalize_advantages", action="store_false")
    ...
    if args.ppo_clip_eps <= 0:
        raise ValueError("--ppo_clip_eps must be positive")
    if args.ppo_log_ratio_clip <= 0:
        raise ValueError("--ppo_log_ratio_clip must be positive")
...
def main() -> None:
    ...
    if not args.slothead_checkpoint:
        raise ValueError("--slothead_checkpoint is required for PPO80; pass a fresh object-mode slothead checkpoint for the current dataset.")
    ...
    value_head = PPOValueHead(args.hidden_dim).to(device)
    ...
    value_optimizer = torch.optim.AdamW(value_head.parameters(), lr=args.lr, weight_decay=args.wd)
    ...
    meta = {
        ...
        "design_note": "Slothead80 supplies u=[u_obj,u_geo,u_res]. PPO80 keeps the GRPO80 SlotSelectorGRPO policy architecture and slot-selection rollout path, replacing only the group-relative GRPO objective with clipped PPO plus a learned value baseline. Reward is label-free: objectness keeps selected slots object-like, geometry rewards objectness-weighted incremental non-overlap coverage, residual novelty rewards non-duplicate u_res evidence. Scene labels are used only in CE loss.",
    }
    (out_dir / "selector_ppo_meta.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_metrics = run_epoch(model, value_head, backbone, projector, train_loader, device, ce_optimizer, policy_optimizer, value_optimizer, args, True)
        args.warmup_epochs_remaining = 0
        with torch.no_grad():
            valid_metrics = run_epoch(model, value_head, backbone, projector, valid_loader, device, ce_optimizer, policy_optimizer, value_optimizer, args, False)
        row = {
            "epoch": epoch,
        ...
        if valid_metrics["acc"] > best_acc:
            ...
            torch.save(
                ...
                out_dir / "selector_ppo_best.pt",
            )
            print(f"saved best selector: {out_dir / 'selector_ppo_best.pt'}")
