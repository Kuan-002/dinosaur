#!/usr/bin/env python3
"""Actor-critic counterpart for the 8-class contrastive rule-graph experiment."""
...
from train_grpo8_contrastive_pair import DATA, SA, auxiliary_loss
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--early_exit_conf", type=float, default=0.90)
...
def rollout_ac(model: SlotSelectorAC, slots: torch.Tensor, labels: torch.Tensor, args, anchor_ids, evidence_ids) -> dict[str, torch.Tensor]:
    ...
    previous_anchor = slots.new_zeros(batch)
    previous_evidence = slots.new_zeros(batch)
    previous_pair_margin = fac.rule_pair_margin(
        component_prob, selected, labels, anchor_ids, evidence_ids
    )
    ...
    for step in range(min(int(args.max_steps), num_slots)):
        ...
        anchor, evidence, pair = fac.selected_support(component_prob, selected, labels, anchor_ids, evidence_ids)
        ...
        reward = (
            ...
            + args.anchor_coef * (anchor - previous_anchor)
            + args.evidence_coef * (evidence - previous_evidence)
            + args.pair_coef * (pair_margin - previous_pair_margin)
        ) * discount
        rewards.append(reward)
        previous_anchor, previous_evidence = anchor, evidence
...
def main() -> None:
    ...
    meta = {
        ...
        "standard": "same contrastive pair reward/forced top-k eval as GRPO8; group advantage replaced by learned value baseline",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, labels in train_loader:
            ...
            rollout = rollout_ac(model, slots, labels, args, anchor_ids, evidence_ids)
            mask = rollout["mask"]
            denom = mask.sum().clamp_min(1.0)
