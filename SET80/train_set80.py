...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if score > best_score + args.early_stop_min_delta:
            ...
            best_epoch = epoch
            stale = 0
            torch.save({"args": vars(args), "probe_config": asdict(cfg), "classes": classes, "epoch": epoch, "valid_selection_score": score, "valid_stats": valid_stats, "model_state_dict": model.state_dict()}, out_dir / "set80_best.pt")
            print(f"saved {out_dir / 'set80_best.pt'}")
        else:
            stale += 1
