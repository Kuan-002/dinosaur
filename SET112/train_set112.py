...
from structured112 import load_structured112, project_slots112
...
def train_epoch(model, backbone, projector, loader, device, optimizer, args) -> dict[str, float]:
    ...
    for images, labels in loader:
        ...
        slots = encode_slots112(backbone, projector, images, device, args.structured_mode)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train 112-dim structured-slot SetTransformer reward probe.")
    parser.add_argument("--data", default=DEFAULT_DATA)
    parser.add_argument("--sa_checkpoint", default=DEFAULT_SA)
    parser.add_argument("--structured_checkpoint", default=DEFAULT_STRUCTURED112)
...
def main() -> None:
    ...
    if not args.structured_checkpoint:
        raise ValueError("--structured_checkpoint is required for SET112; train or pass a 112-dim structured bottleneck checkpoint.")
    ...
    projector, structured_ckpt = load_structured112(args.structured_checkpoint, device)
    slot_dim = int(projector.cfg.out_dim if args.structured_mode == "u" else encode_slots112(backbone, projector, next(iter(valid_loader))[0], device, args.structured_mode).size(-1))
    ...
    meta = {"args": vars(args), "probe_config": asdict(cfg), "classes": classes, "structured112_config": structured_ckpt["config"], "model_class": "settransformer.model.DiscriminativeSetTransformer"}
