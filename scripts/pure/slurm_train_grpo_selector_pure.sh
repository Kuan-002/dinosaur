#!/bin/bash
#SBATCH --job-name=grpo_selector_pure
...
RUN_NAME=${RUN_NAME:-grpo_selector_pure_minstep2_free2_count020_stepdelta_$(date +%Y%m%d_%H%M%S)}
...
STEP_MARGIN_REWARD_WEIGHTS=${STEP_MARGIN_REWARD_WEIGHTS:-1.0,0.5,0.3}
GRPO_GROUP_SIZE=${GRPO_GROUP_SIZE:-4}
FREE_SLOTS=${FREE_SLOTS:-2}
COUNT_PENALTY=${COUNT_PENALTY:-0.2}
