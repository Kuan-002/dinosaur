#!/bin/bash
#SBATCH --job-name=grpo_selector_s45_c03
...
PROBE_CKPT=${PROBE_CKPT:-/vol/biomedic3/kw1025/dinosaur/checkpoints/set_probe_top2_clean10_anchor009_evidence005_20260703_195936/set_transformer_slot_probe_best.pt}
RUN_NAME=${RUN_NAME:-grpo_selector_pure_slots4_5_contrast03_minstep2_free3_count045_stepdelta_$(date +%Y%m%d_%H%M%S)}
...
SUBSET_CONTRAST_WEIGHT=${SUBSET_CONTRAST_WEIGHT:-0.3}
STEP_MARGIN_REWARD_WEIGHTS=${STEP_MARGIN_REWARD_WEIGHTS:-0.4,0.15,0.05}
GRPO_GROUP_SIZE=${GRPO_GROUP_SIZE:-4}
FREE_SLOTS=${FREE_SLOTS:-3}
COUNT_PENALTY=${COUNT_PENALTY:-0.45}
...
echo "Frozen SetTransformer reward probe: ${PROBE_CKPT}"
