...
def collect_candidates(
    ...
    for source_split in ["train", "val"]:
        ...
        for image_id, info in images.items():
            ...
            matches = []
            for concept in concepts:
                anchors = [obj for obj in concept["anchors"] if ratios.get(obj, 0.0) >= min_anchor_area_ratio]
                evidence = [obj for obj in concept["evidence"] if ratios.get(obj, 0.0) >= min_evidence_area_ratio]
                if anchors and evidence:
                    matches.append((concept, anchors[0], evidence[0]))
            if len(matches) > 1:
                ambiguous += 1
                continue
            if not matches:
                continue
            concept, anchor_obj, evidence_obj = matches[0]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a 10-class COCO dataset from two-object scene concepts.")
    parser.add_argument("--coco_root", default="dataset/coco2017")
    parser.add_argument("--out_dir", required=True)
    parser.add_argument("--min_anchor_area_ratio", type=float, default=0.07)
    ...
    candidates, ambiguous_count = collect_candidates(
        coco_root,
        args.min_anchor_area_ratio,
        args.min_evidence_area_ratio,
        concepts,
    )
    ...
    summary = {
        ...
        "label_definition": "10 scene-concept classes requiring one anchor object and one distinct evidence object/group",
        "min_anchor_area_ratio": args.min_anchor_area_ratio,
        "min_evidence_area_ratio": args.min_evidence_area_ratio,
        "ambiguous_candidate_count_dropped": ambiguous_count,
