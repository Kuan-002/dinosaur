#!/bin/bash
#SBATCH --job-name=grpo_settr_policy_stop
...
RUN_NAME=${RUN_NAME:-grpo_selector_disc_settr174127_policy_stop_no_confexit_m2_free3_count035_stepdelta_$(date +%Y%m%d_%H%M%S)}
...
echo "Stop mode: policy stop action only; confidence early exit disabled."
...
VIZ_PER_CLASS_CORRECT=${VIZ_PER_CLASS_CORRECT:-7}
VIZ_PER_CLASS_WRONG=${VIZ_PER_CLASS_WRONG:-3}
VIZ_OUT_DIR=${VIZ_OUT_DIR:-${OUT_DIR}/visualizations/test_slot_paths_${VIZ_PER_CLASS_CORRECT}c_${VIZ_PER_CLASS_WRONG}w_policy_stop}
