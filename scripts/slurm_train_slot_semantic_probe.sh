#!/usr/bin/env bash
#SBATCH --job-name=slot_sem_probe
...
RUN_NAME="${RUN_NAME:-slot_semantic_probe_cov030_pur020_$(date +%Y%m%d_%H%M%S)}"
...
"${PYTHON}" analysis/slothead/train_slot_semantic_probe.py \
  --data "${DATA}" \
  --coco_root "${COCO_ROOT}" \
...
echo "Checkpoint:"
echo "${OUT_DIR}/slot_semantic_probe_best.pt"
