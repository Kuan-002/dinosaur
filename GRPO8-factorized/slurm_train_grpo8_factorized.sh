...
RUN_NAME="${RUN_NAME:-grpo8_factorized_joint_seed8_$(date +%Y%m%d_%H%M%S)}"
...
"${PYTHON:-.venv/bin/python}" GRPO8-factorized/train_grpo8_factorized.py \
  ...
  --pair_ce_coef "${PAIR_CE_COEF:-1.0}" --anchor_coef "${ANCHOR_COEF:-0.25}" \
  --evidence_coef "${EVIDENCE_COEF:-2.0}" --pair_coef "${PAIR_COEF:-1.0}" \
  --rule_margin_coef "${RULE_MARGIN_COEF:-0.5}" --completion_bonus "${COMPLETION_BONUS:-0.5}" \
