"""Shared factorized-MIL rewards and rollout utilities for GRPO8.

This module is intentionally local to GRPO8 so the experiment is reproducible
without the removed ``GRPO10-factorized`` experiment directory.
"""
...
def rollout(
    ...
    for step in range(min(int(args.max_steps), num_slots)):
        ...
        pair_reward_steps.append((pair_support - previous_pair_support) * discount)
        class_logits = model.classify(hidden)
    ...
    for step in range(max(top_ks)):
        ...
        if k in top_ks:
            ...
            output[k] = {
                ...
                "pair": pair_support,
                "pair_hit": (anchor_support >= 0.5) & (evidence_support >= 0.5),
