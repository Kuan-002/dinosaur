...
@torch.no_grad()
def encode_slots_full(backbone, images: torch.Tensor, device: torch.device) -> torch.Tensor:
...
def train_epoch(model, backbone, loader, device, optimizer, args) -> dict[str, float]:
    ...
    for images, labels in loader:
        ...
        slots = encode_slots_full(backbone, images, device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SetTransformer over raw DINOSAUR slot embeddings.")
    parser.add_argument("--data", default=DEFAULT_DATA)
    parser.add_argument("--sa_checkpoint", default=DEFAULT_SA)
    parser.add_argument("--output_dir", default="SET_full/checkpoints/set_full")
...
def main() -> None:
    ...
    slot_dim = int(backbone.slot_dim)
    ...
    meta = {
        ...
        "classes": classes,
        "slot_embedding_source": "dinosaur_raw",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if score > best_score + args.early_stop_min_delta:
            ...
            torch.save(
                ...
                out_dir / "set_full_best.pt",
            )
            print(f"saved {out_dir / 'set_full_best.pt'}")
        else:
            stale += 1
