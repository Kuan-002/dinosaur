...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train 56-dim structured-slot SetTransformer reward probe.")
...
def main() -> None:
    ...
    slot_dim = int(projector.cfg.out_dim if args.structured_mode == "u" else encode_slots56(backbone, projector, next(iter(valid_loader))[0], device, args.structured_mode).size(-1))
    ...
    meta = {"args": vars(args), "probe_config": asdict(cfg), "classes": classes, "structured56_config": structured_ckpt["config"], "model_class": "settransformer.model.DiscriminativeSetTransformer"}
