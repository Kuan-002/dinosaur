...
from slothead56 import load_slothead56, project_slots56
...
def train_epoch(model, backbone, projector, loader, device, optimizer, args) -> dict[str, float]:
    ...
    for images, labels in loader:
        ...
        slots = encode_slots56(backbone, projector, images, device, args.slothead_mode)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train 56-dim slothead SetTransformer probe.")
    parser.add_argument("--data", default=DEFAULT_DATA)
    parser.add_argument("--sa_checkpoint", default=DEFAULT_SA)
    parser.add_argument("--slothead_checkpoint", default=DEFAULT_SLOTHEAD56)
    parser.add_argument("--slothead_mode", choices=["u", "obj", "geo", "res", "obj_geo", "obj_res", "geo_res"], default="u")
    parser.add_argument("--output_dir", default="SET56/checkpoints/set56")
...
def main() -> None:
    ...
    if not args.slothead_checkpoint:
        raise ValueError("--slothead_checkpoint is required for SET56; pass a fresh object-mode slothead checkpoint for the current dataset.")
    ...
    projector, slothead_ckpt = load_slothead56(args.slothead_checkpoint, device)
    slot_dim = int(projector.cfg.out_dim if args.slothead_mode == "u" else encode_slots56(backbone, projector, next(iter(valid_loader))[0], device, args.slothead_mode).size(-1))
    ...
    meta = {"args": vars(args), "probe_config": asdict(cfg), "classes": classes, "slothead56_config": slothead_ckpt["config"], "model_class": "settransformer.model.DiscriminativeSetTransformer"}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if score > best_score + args.early_stop_min_delta:
            ...
            best_epoch = epoch
            stale = 0
            torch.save({"args": vars(args), "probe_config": asdict(cfg), "classes": classes, "epoch": epoch, "valid_selection_score": score, "valid_stats": valid_stats, "model_state_dict": model.state_dict()}, out_dir / "set56_best.pt")
            print(f"saved {out_dir / 'set56_best.pt'}")
        else:
            stale += 1
